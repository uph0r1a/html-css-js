...
class ShoppingCart {
  ...
  addItem(id, products) {
    ...
    const counts = {};
    this.items.forEach(
      (item) => (counts[item.id] = (counts[item.id] || 0) + 1)
    );

    const currentCount = counts[product.id];
    const countSpanId = `product-count-for-id${id}`;
    let countSpan = document.getElementById(countSpanId);

    if (currentCount > 1) {
      if (countSpan) countSpan.textContent = `${currentCount}x `;
    } else {
      productsContainer.innerHTML += `
                <div id="dessert${id}" class="product">
                    <p>
                        <span class="product-count" id="${countSpanId}">${
  ...
  clearCart() {
    if (!this.items.length) return alert("Your shopping cart is already empty");
