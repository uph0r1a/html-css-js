...
const checkpoints = checkpointPositions.map(
  (cp) => new CheckPoint(cp.x, cp.y, cp.z)
);
...
const animate = () => {
  ...
  platforms.forEach((platform) => {
    ...
    if (sideRules.every(Boolean)) {
      player.position.y = platform.position.y + player.height;
      player.velocity.y = gravity;
    }
  });

  checkpoints.forEach((checkpoint, index, arr) => {
    const rules = [
      ...
      index === 0 || arr[index - 1].claimed === true,
    ];
    ...
    if (rules.every(Boolean)) {
      ...
      if (index === arr.length - 1) {
        ...
        showCheckpointScreen("Checkpoint complete! You reached the end!");
        movePlayer("ArrowRight", 0, false);
      } else if (
        player.position.x >= checkpoint.position.x &&
        player.position.x <= checkpoint.position.x + 40
      ) {
        showCheckpointScreen("Checkpoint activated!");
      }
    }
  });
};
